...
function mostrarUsuario() {
    ...
    // Si falta algún elemento en el HTML, no hacemos nada para evitar errores
    if (!btnLogin || !divUsuario) return;

    if (user) {
        // Usuario Logueado: Ocultar botón login, mostrar nombre
        btnLogin.style.display = "none";
        divUsuario.style.display = "flex";
        divUsuario.style.alignItems = "center";
        if (spanUsername) spanUsername.textContent = user;
    } else {
        // Usuario No Logueado: Mostrar botón login, ocultar nombre
...
async function refrescarToken() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Si el token expiró, sacamos al usuario
            cerrarSesion();
        }
    } catch (error) {
